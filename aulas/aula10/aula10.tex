...
\begin{slide}
  ...
  \item L. Gonda: Algoritmos BSP/CGM para ordenação, dissertação de mestrado, UFMS, 2004.
   \end{enumerate}
\end{slide}
...
~~~~ \= ~~~~\= ~~~~\= ~~~~\= ~~~~~~~\= \hspace*{12cm}                               \= \\
 ...
 16 \>\>\>$S_k' \leftarrow Ordene(S'_{k-1} \cup R_k)$\\
...
Um \textbf{\darkgreen r-ruling set} de L é um subconjunto de elementos selecionados da lista $L$ com as seguintes propriedades:
  ...
  \vspace{-1cm} \hspace{1cm} (2) A distância entre qualquer elemento não selecionado ao próximo elemento selecionado é no máximo $r$.  
\end{slide}
...
\textbf{se} ($i, s[i]$ e $s[s[i]]$ são selecionado) \textbf{e não} ($l(i) < l(s[i]) > l(s[s[i]])$) \textbf{e} ($l(s[i]) \neq l(s[s[i]])$) \textbf{então} 
